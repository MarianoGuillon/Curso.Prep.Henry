  ...
  for(let i=0; i<string.length; i++){
    ...
    for(let clave in nuevoObjeto){
      if(clave === string.charAt(i)){
        let num = nuevoObjeto[clave];
        nuevoObjeto[clave] = num+1;
        bandera+=1;
        break;
      }
    }
    if(bandera===0){
      nuevoObjeto[string.charAt(i)]=1;
    }
  }
  return nuevoObjeto;
}
...
function capToFront(s) {
  ...
  for(let i=0; i<s.length;i++){
    if(s.charAt(i).toUpperCase() === s.charAt(i)){
      stringFinal = stringFinal+s.charAt(i); 
    }
  }
  for(let i=0; i<s.length;i++){
...
function asAmirror(str) {
  ...
  for(let i=0;i<strArray.length;i++){
    for(let j=strArray[i].length-1;j>=0;j--){
      palabra=palabra+strArray[i].charAt(j);
    }
    palabra = palabra + " ";
  }
